...
class SnapearnPointQuery extends \yii\db\ActiveQuery
{
    ...
    public function getReportDetail($id,$date)
    {
        ...
        $last = WorkingTime::find()
                ...
                ->one()->wrk_id;
        $this->select([
            'tbl_working_time.wrk_type',
            'tbl_working_time.wrk_start',
            'tbl_working_time.wrk_end',
            'tbl_working_time.wrk_time',
            'tbl_working_time.wrk_point',
            'spo_name',
            'spo_point'
            ]);
        $this->leftJoin('tbl_working_time', 'tbl_working_time.wrk_rjct_number = spo_id');
...
//        \yii\helpers\VarDumper::dump($this->asArray()->all(),10,true);exit;
