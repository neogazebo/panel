    ...
    <tr>
        <th class="banner" style="text-align: left;vertical-align: middle;padding: 3px 18px;">
             <?php if($data['model']->company->com_registered_to == User::REGISTERED_TO_TM){?>
                <img style="width: 100%;margin-left: -10px;" src="<?= Yii::$app->urlManager->hostInfo ?>/img/header-invoice.jpg" />
            <?php }else if($data['model']->company->com_registered_to == 'RHB'){?>
            	<img style="width: 100%;margin-left: -10px;" src="<?= Yii::$app->urlManager->hostInfo ?>/img/header-invoice-rhb.jpg" />
			<?php }else{?>
...
<table style="width: 100%;font: 12px 'Arial';line-height: 20px;" cellspacing="0" cellpadding="0">
    ...
    <?php 
    $mySubscriptions = $data['model']->company->mySubscriptions;
